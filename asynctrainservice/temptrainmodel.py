...
class BaseModel():
    ...
    def save(self, is_check = False, history = None, is_training= False, eta=0, origin='none'):
        ...
        if not is_check and self.model:
            self.model.save(os.path.join(self.model_path, 'model_new.h5'))
    ...
    @staticmethod
    def trim_only_general_thai_viet_chinese(string):
        _result = ''
        ...
        for uc in string:
            ...
            # Thai
            elif _code >= 0x0e00 and _code <= 0x0e7f:
                _result += uc
                continue
            
            # Vietnamese
            elif (_code >= 0x0080 and _code <= 0x024f) or \
                (_code >= 0x02c6 and _code <= 0x036f) or \
                (_code >= 0x1dc0 and _code <= 0x1eff):
                _result += uc
                continue
            ...
            if _code >= 0x0000 and _code <= 0x007f:
                _result += chr(_code)
...
class ChineseChatModel(BaseModel):
    ...
    def fit_model(self, verbose=0, callback=None):
        ...
        logging.info('reading chinese training data...')
        self.data = pd.read_csv(os.path.join(CH_TRAIN_DATA_PATH, 'ai_textbooksentence_2025_06_25.csv'))
        ...
        x_train, x_val, y_train, y_val = train_test_split(x_ch, y_ch, stratify=y_ch, test_size=0.2, random_state=42)
        ...
        # thai_train_df['text'] = thai_train_df['text'].apply(lambda x: ChineseChatModel.trim_only_general_thai_viet_chinese(x).strip())
        ...
        # thai_val_df['text'] = thai_val_df['text'].apply(lambda x: ChineseChatModel.trim_only_general_thai_viet_chinese(x).strip())
        ...
        # vi_train_df['text'] = vi_train_df['text'].apply(lambda x: ChineseChatModel.trim_only_general_thai_viet_chinese(x).strip())
        ...
        # vi_val_df['text'] = vi_val_df['text'].apply(lambda x: ChineseChatModel.trim_only_general_thai_viet_chinese(x).strip())
        ...
        logging.info('total length of indonesian training data: {}'.format(len(ind_train_df.index)))
        ...
        logging.info('total length of indonesian val data: {}'.format(len(ind_val_df.index)))
    ...
    def transform_data(self, df):
        ...
        # df['text'] = df['text'].apply(lambda x: ChineseChatModel.trim_only_general_thai_viet_chinese(x).strip())
...
class ChineseNicknameModel(BaseModel):
    ...
    def fit_model(self, verbose=0, callback=None):
        ...
        # self.data = self.get_train_data(config.get('TRAINING_DATA_TABLE', 'CHAT'))
        logging.info('reading chinese training data...')
        self.data = pd.read_csv(os.path.join(CH_TRAIN_DATA_PATH, 'ai_nicknametextbook_2025_06_25.csv'))
        self.data = self.data[['text', 'status']]
        self.data = self.data.dropna()
        ...
        # thai_train_df['text'] = thai_train_df['text'].apply(lambda x: ChineseNicknameModel.trim_only_general_thai_viet_chinese(x).strip())
        thai_train_df = thai_train_df[thai_train_df['text'].astype(bool)] # remove rows containing empty string
        thai_train_df = thai_train_df.drop_duplicates(subset=['text'])
        logging.info('total length of thai training data: {}'.format(len(thai_train_df.index)))

        # thai_val_df['text'] = thai_val_df['text'].apply(lambda x: ChineseNicknameModel.trim_only_general_thai_viet_chinese(x).strip())
        thai_val_df = thai_val_df[thai_val_df['text'].astype(bool)] # remove rows containing empty string
        thai_val_df = thai_val_df.drop_duplicates(subset=['text'])
        logging.info('total length of thai val data: {}'.format(len(thai_val_df.index)))

        # thai_train_df['target'] = thai_train_df['status'].apply(lambda x: x if x < 2 else 2)
        # thai_val_df['target'] = thai_val_df['status'].apply(lambda x: x if x < 2 else 2)
        ...
        # vi_train_df['text'] = vi_train_df['text'].apply(lambda x: ChineseNicknameModel.trim_only_general_thai_viet_chinese(x).strip())
        vi_train_df = vi_train_df[vi_train_df['text'].astype(bool)] # remove rows containing empty string
        vi_train_df = vi_train_df.drop_duplicates(subset=['text'])
        logging.info('total length of viet training data: {}'.format(len(vi_train_df.index)))

        # vi_val_df['text'] = vi_val_df['text'].apply(lambda x: ChineseNicknameModel.trim_only_general_thai_viet_chinese(x).strip())
        vi_val_df = vi_val_df[vi_val_df['text'].astype(bool)] # remove rows containing empty string
        vi_val_df = vi_val_df.drop_duplicates(subset=['text'])
        logging.info('total length of viet val data: {}'.format(len(vi_val_df.index)))

        # vi_train_df['target'] = vi_train_df['status'].apply(lambda x: x if x < 2 else 2)
        # vi_val_df['target'] = vi_val_df['status'].apply(lambda x: x if x < 2 else 2)
    ...
    def transform_data(self, df):
        ...
        df['text'] = df['text'].apply(lambda x: self.cc.convert(x))
        df['text'] = df['text'].apply(lambda x: preprocess_text(x))
        # df['text'] = df['text'].apply(lambda x: ChineseNicknameModel.trim_only_general_thai_viet_chinese(x).strip())
        df = df[df['text'].astype(bool)] # remove rows containing empty string
        df = df.drop_duplicates(subset=['text'])
        return df
