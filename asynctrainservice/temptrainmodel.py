...
class ChineseChatModel(BaseModel):
    ...
    def fit_model(self, verbose=0, callback=None):
        ...
        self.data = pd.read_csv(os.path.join(CH_TRAIN_DATA_PATH, 'ai_textbooksentense_2024_10_20.csv'))
...
class ChineseNicknameModel(BaseModel):
    ...
    def fit_model(self, verbose=0, callback=None):
        ...
        # self.data = self.get_train_data(config.get('TRAINING_DATA_TABLE', 'CHAT'))
        logging.info('reading chinese training data...')
        self.data = pd.read_csv(os.path.join(CH_TRAIN_DATA_PATH, 'ai_nicknametextbook_2024_10_20.csv'))
        self.data = self.data[['text', 'status']]
        self.data = self.data.dropna()
