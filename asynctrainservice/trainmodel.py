...
class BaseModel():
    ...
    def get_test_batchs(self):
        ...
        test_dataset = tf.data.Dataset.from_tensor_slices(np.stack(X)).batch(BATCH_SIZE).prefetch(2)
...
class ChineseChatModel(BaseModel):
    ...
    def fit_model(self, verbose=0, callback=None):
        ...
        try:
            weights_file = os.path.join(self.model_path, "transformer_best_weights.h5")
